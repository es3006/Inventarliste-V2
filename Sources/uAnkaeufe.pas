...
var
  ...
  sku, ref, marke, model, bezeichnung, gewicht: string;
  box, papiere, jahr, savedInInventar: Integer;
...
begin
  ...
  if not IsValidDecimalString(edKaufpreis.Text) then
  begin
    ShowMessage('Ungültiger Kaufpreis! Bitte verwenden Sie das Format: 123,45');
    edKaufpreis.SetFocus;
    Exit;
  end;
  ...
  if (Trim(edGesamtpreis.Text) <> '') and not IsValidDecimalString(edGesamtpreis.Text) then
  begin
    ShowMessage('Ungültiger Gesamtpreis! Bitte verwenden Sie das Format: 135,95');
    edGesamtpreis.SetFocus;
    Exit;
  end;
  ...
  if not TryStrToInt(Trim(edJahr.Text), jahr) or (jahr <= 0) then
  begin
    ShowMessage('Bitte geben Sie ein gültiges Herstellungsjahr ein!');
    edJahr.SetFocus;
    Exit;
  end;
  ...
  savedInInventar := Ord(cbSavedInInventar.Checked);
  ...
  // ===== Beträge mit Helper-Funktionen =====
  ankaufspreis := DecimalStringToInt100(edKaufpreis.Text);
  versandpreis := DecimalStringToInt100(edVersand.Text);
  gesamtpreis := DecimalStringToInt100(edGesamtpreis.Text);
  ...
  try
    ...
    try
      ...
      SetSQLiteDateParam(
        FDQuery.ParamByName('ANKAUFDATUM'),
        dtpAnkaufsdatum
      );
...
begin
  ...
  try
    ...
    try
      ...
      Q.SQL.Text :=
        'SELECT A.id, A.SKU, K.KundenNr, K.Nachname, K.Vorname, ' +
        'K.StrasseHausNr, K.PLZ, K.Ort, A.Einkaufsdatum, A.EinkaufBemerkung, ' +
        'A.Einkaufswert, A.Versand, A.Ref, A.Box, A.Papiere ' +
        'FROM inventar A ' +
      ...
      begin
        ...
        // === Einkaufsdatum mit Helper-Funktion ===
        Item.SubItems.Add(SQLiteDateToDisplay(Q.FieldByName('Einkaufsdatum').AsString));

        Item.SubItems.Add(Q.FieldByName('EinkaufBemerkung').AsString);
        Item.SubItems.Add(Int100ToDecimalString(Q.FieldByName('Einkaufswert').AsLargeInt));
...
begin
  ...
  try
    ...
    Q.SQL.Text :=
      ...
      'Gewicht, Karat, Ref, Box, Papiere, Marke, Model, ' +
      'Jahr, Zustand, Versand, Zahlungsart ' +
    ...
    begin
      ...
      edKaufpreis.Text := Int100ToDecimalString(Q.FieldByName('Einkaufswert').AsLargeInt);
      edVersand.Text := Int100ToDecimalString(Q.FieldByName('Versand').AsLargeInt);
      edRef.Text := Q.FieldByName('Ref').AsString;
      edGewicht.Text := Int100ToDecimalString(Q.FieldByName('Gewicht').AsLargeInt);
