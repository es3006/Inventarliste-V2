...
uses
  ...
  System.Generics.Collections, System.Classes, DateUtils, ShellApi, System.Threading, System.IOUtils, inifiles,
...
begin
  GetRemoteVersionAsync(UPDATEURL + VERSION_FILE,
    ...
    begin
      //Keine Versionsinfo abrufbar
      if RemoteVersion = '' then
      begin
        MessageDlg('Die Update-Version konnte nicht ermittelt werden.',mtWarning, [mbOK], 0);
        Exit;
      end;

      //KEIN UPDATE VERFÜGBAR
      if VersionCompare(VersionInstalled, RemoteVersion) >= 0 then
      begin
        MessageDlg(
          'Es ist kein Update verfügbar.' + sLineBreak +
          'Installierte Version: ' + VersionInstalled,
      ...
      GetChangeLogAsync(UPDATEURL + CHANGELOG_FILE,
        ...
        begin
          if MessageDlg(
               'Ein Update auf Version ' + RemoteVersion + ' ist verfügbar.' + sLineBreak +
               sLineBreak +
               'Änderungen:' + sLineBreak +
               Changelog + sLineBreak + sLineBreak +
               'Möchten Sie das Update jetzt installieren?',
               mtInformation, [mbYes, mbNo], 0) = mrYes then
          begin
            ini := TIniFile.Create(PATH + 'settings.ini');
            try
              ini.DeleteKey('UPDATE', 'InstallUpdateQuestion');
            finally
              ini.Free;
            end;
          ...
          begin
            ini := TIniFile.Create(PATH + 'settings.ini');
            try
              ini.WriteInteger('UPDATE', 'InstallUpdateQuestion', Trunc(Now));
            finally
              ini.Free;
            end;
          end;
  ...
  begin
    try
      ...
      begin
        ...
        begin
         // TLogging.LogMessage(BEDIENERUSERNAME, 'uUpdate', 'ExecuteWithRetryBoolean',
  ...
  begin
    try
      ...
      begin
        ...
        begin
       //   TLogging.LogMessage(BEDIENERUSERNAME, 'uUpdate', 'ExecuteWithRetryString',
       //     Format('Versuch %d fehlgeschlagen, wiederhole in %dms: %s', [Attempt, NETWORK_RETRY_DELAY, E.Message]));
          Sleep(NETWORK_RETRY_DELAY);
        end;
      end;
...
begin
  ...
  OldExe := AppDir + 'Inventarliste.exe';
  NewExe := AppDir + 'Inventarliste.tmp';
  BackupExe := AppDir + 'Inventarliste_old.exe';
...
begin
  ...
  var
    ...
    StartTime: TDateTime;
  begin
    DateiURL := UPDATEURL + 'Inventarliste.exe';
    UpdatePfad := TPath.Combine(PATH, 'Inventarliste.tmp');
    Fehler := '';
    ...
    begin
      ...
      RemoteHash := LadeRemoteSHA256(UPDATEURL + 'Inventarliste.sha256.txt');
...
begin
  TTask.Run(
    ...
    begin
      ...
      // 🔑 Callback IMMER in den MainThread
      TThread.Queue(nil,
        procedure
        begin
