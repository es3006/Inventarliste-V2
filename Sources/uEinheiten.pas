...
begin
  ...
  if Trim(edSortierung.Text) = '' then
  begin
    ShowMessage('Bitte geben Sie den Wert in Gramm ein!');
    Exit;
  end;
  ...
  sortierung := StrToInt(Trim(edSortierung.Text));

  FDQuery := TFDQuery.Create(nil);
  try
    FDQuery.Connection := fMain.FDConnection1;

    FDQuery.SQL.Text := 'INSERT INTO einheiten (einheit, sortierung) VALUES (:EINHEIT, :SORTIERUNG)';

    fMain.FDConnection1.StartTransaction;
    try
      FDQuery.ParamByName('EINHEIT').AsString  := einheit;
      FDQuery.ParamByName('SORTIERUNG').AsInteger  := sortierung;
      FDQuery.ExecSQL;
...
begin
  ...
  if Trim(edSortierung.Text) = '' then
  begin
    ShowMessage('Bitte geben Sie eine Zahl ein nach der sortiert werden soll!');
    Exit;
  end;
  ...
  try
    ...
    FDQuery.SQL.Text := 'UPDATE einheiten set einheit = :EINHEIT, sortierung = :SORTIERUNG WHERE id = :ID';
  ...
  try
    ...
    try
      ...
      begin
        ...
        Item.SubItems.Add(Q.FieldByName('Sortierung').AsString);
...
begin
  ...
  if(i<>-1) then
  begin
    edEinheit.Text := lvEinheiten.Items[i].SubItems[0];
    edSortierung.Text := lvEinheiten.Items[i].SubItems[1];
    btnSave.Caption := 'Speichern';
    btnDelete.Enabled := true;
    btnDelete.Visible := true;
  end
