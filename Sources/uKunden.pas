...
type
  TfKunden = class(TForm)
    ...
    procedure lvKundenSelectItem(Sender: TObject; Item: TListItem; Selected: Boolean);
...
uses
  uMain, uDBFunctions, uSQLiteDateHelper;
...
begin
  ...
  if dtpGeburtsdatum.Checked then
    GeburtsdatumStr := DateTimeToSQLiteDate(dtpGeburtsdatum.Date)
  else
    GeburtsdatumStr := '0';
  ...
  try
    ...
    try
      ...
      with FDQuery.ParamByName('AUSWEISNR') do
      begin
        DataType := ftString;
        if ausweisnr = '' then
          Clear        // → NULL
      ...
      FDQuery.ParamByName('GEBURTSDATUM').AsString := GeburtsdatumStr;
  ...
  try
    ...
    try
      ...
      begin
        ...
        Item.SubItems.Add(SQLiteDateToDisplay(Q.FieldByName('Geburtsdatum').AsString));
...
begin
  ...
  begin
    ...
    LoadDisplayDateToDTP(lvKunden.Items[i].SubItems[10], dtpGeburtsdatum);
  end
  else
  begin
    pnlKundendaten.Visible := false;
