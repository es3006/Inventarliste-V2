...
type
  TfMain = class(TForm)
    ...
    acExportCSV: TAction;
    Panel2: TPanel;
    acHinweisfenster: TAction;
    pnlInventarliste: TPanel;
    lvInventar: TAdvListView;
    pnlEdelmetallAnVerkauf: TPanel;
    Panel3: TPanel;
    cbShowEdelmetallAnVerkauf: TCheckBox;
    Panel4: TPanel;
    Panel6: TPanel;
    Label5: TLabel;
    lbEinkaeufeGesamtCount: TLabel;
    Label7: TLabel;
    lbVerkaeufeGesamtCount: TLabel;
    pcEdelmetallAnVerkaeufe: TAdvPageControl;
    AdvTabSheet1: TAdvTabSheet;
    AdvTabSheet2: TAdvTabSheet;
    lvEdelmetall: TAdvListView;
    lvEdelmetallVerkauf: TAdvListView;
    Panel5: TPanel;
    btnEdelmetallVerkauf: TButton;
    cbCheckUncheckAll: TCheckBox;
    ...
    procedure acHinweisfensterExecute(Sender: TObject);
    procedure cbShowEdelmetallAnVerkaufClick(Sender: TObject);
    procedure btnEdelmetallVerkaufClick(Sender: TObject);
    procedure cbCheckUncheckAllClick(Sender: TObject);
    procedure lvEdelmetallCheckboxClick(Sender: TObject; iItem: Integer; State: Boolean);
    procedure lvEdelmetallVerkaufDblClick(Sender: TObject);
  ...
  private
    ...
    procedure LoadEdelmetallAnkaufToListView;
    procedure LoadEdelmetallVerkaufToListView;
  public
    procedure LoadStatusBarValues;
    procedure AktualisiereAndShowStatistik;
    procedure LoadInventarToListView;
  end;

var
  fMain: TfMain;
  PATH, PATHHELP, PATHSCRIPTS, PATHTEMP, PATHDOKUMENTE, PATHANKAUFSFORMULARE: string;
  PATHANKAUFSFORMULAREEDELMETALL, DBNAME, PROGRAMMNAME, PROGRAMMVERSION: string;
  ...
  ListViewDirty, SHOWHINWEISFENSTER, SHOWEDELMETALLANVERKAUFWINDOW: Boolean;
...
uses
  uLizenzDialog, uFunctions, uDBFunctions, uEinheiten, uVerkauf, uEditWareneinkauf,
  ...
  uEdelmetallsammelverkauf, uMoneyHelper, uSQLiteDateHelper, uWareneinkauf,
  uSplash, uSammelverkaeufe, uPreisabfrage;
...
begin
  ...
  try
    ...
    begin
      ...
      if (S <> '') and (not IsValidDisplayDate(S)) then
      begin
      ...
      if not IsValidDecimalString(S) then
      begin
      ...
      if (S <> '') and (not IsValidDecimalString(S)) then
      begin
...
var
  ini: TIniFile;
begin
  SHOWHINWEISFENSTER := not SHOWHINWEISFENSTER;

  ini := TIniFile.Create(PATH + 'settings.ini');
  try
    ini.WriteBool('ANSICHT','ShowDownHintWindow',SHOWHINWEISFENSTER);
  finally
    ini.Free;
  end;
...
procedure TfMain.btnEdelmetallVerkaufClick(Sender: TObject);
var
  i: Integer;
  IDs: TList<Integer>;
begin
  ...
  try
    for i := 0 to lvEdelmetall.Items.Count - 1 do
      if lvEdelmetall.Items[i].Checked then
        IDs.Add(StrToInt(lvEdelmetall.Items[i].SubItems[0]));

    if IDs.Count = 0 then
    begin
      ShowMessage('Keine Einträge ausgewählt.');
      Exit;
    end;

    fEdelmetallSammelverkauf := TfEdelmetallSammelverkauf.Create(nil);
    try
      fEdelmetallSammelverkauf.SetAnkaufIDs(IDs);

      if fEdelmetallSammelverkauf.ShowModal = mrOk then
      begin
        LoadEdelmetallAnkaufToListView;
        LoadEdelmetallVerkaufToListView;
      end;

    finally
      fEdelmetallSammelverkauf.Free;
    end;
...
begin
  ...
  begin
    case i of
      ...
      3..5:
      begin
        s := edSuchbegriff.Text;
        LoadInventarToListViewBySuchbegriff(cbSuchfeld.Text, s);
      end;
      6:
      begin
       //LoadInventarToListViewByEdelmetallSammelverkauf;
      end
    else
     s := '';
    end
...
procedure TfMain.cbShowEdelmetallAnVerkaufClick(Sender: TObject);
var
  ini: TIniFile;
begin
  pnlEdelmetallAnVerkauf.Visible := cbShowEdelmetallAnVerkauf.checked;

  if(pnlEdelmetallAnVerkauf.Visible) then
  begin
     LoadEdelmetallAnkaufToListView;
     LoadEdelmetallVerkaufToListView;
  end;
  ...
  ini := TiniFile.Create(PATH + 'settings.ini');
  try
    ini.WriteBool('ANSICHT', 'showEdelmetallAnVerkaufWindow', cbShowEdelmetallAnVerkauf.Checked);
  finally
    ini.Free;
  end;

  pcEdelmetallAnVerkaeufe.ActivePageIndex := 0;
end;
...
begin
  ...
  begin
    edSuchbegriff.Clear;
    case i of
      1..2: // Einkaufsdatum, Verkaufsdatum
      begin
      ...
      6: // Für Sammelverkauf vorgemerkt
      begin
        dtpSuchdatum.Visible := False;
        edSuchbegriff.Visible := False;
        btnSuche.Enabled := True;
...
procedure TfMain.cbCheckUncheckAllClick(Sender: TObject);
var
  i: Integer;
begin
  ...
  lvEdelmetall.Items.BeginUpdate;
  try
    for i := 0 to lvEdelmetall.Items.Count - 1 do
      lvEdelmetall.Items[i].Checked := cbCheckUncheckAll.Checked;
  finally
    lvEdelmetall.Items.EndUpdate;
...
var
  ini: TIniFile;
begin
  // Timer deaktivieren, damit OnTimer-Handler nicht nach der Formularfreigabe ausgeführt werden
  try
    if Assigned(tmHinweis) then
      tmHinweis.Enabled := False;
  except
  end;
  try
    if Assigned(tmStatusBar) then
...
begin
  ...
  PATHANKAUFSFORMULAREEDELMETALL := TPath.Combine(PATH, 'Dokumente\Ankaufsformulare\Edelmetalle');
  ...
  ForceDirectories(PATH + 'Dokumente\Ankaufsformulare\Edelmetalle');
...
begin
  ...
  begin
    ...
    // Beim ersten Aufruf: Caption setzen
    ...
    // Nur laden wenn ListView leer ist
    if lvInventar.Items.Count = 0 then
    begin
      LoadInventarToListView;

      if SHOWEDELMETALLANVERKAUFWINDOW then
      begin
        LoadEdelmetallAnkaufToListView;
        LoadEdelmetallVerkaufToListView;
      end;
    ...
    // ===== WICHTIG: Initialisierung abgeschlossen! =====
    FIsLoading := False;      // ← NEU!
    FInitComplete := True;    // ← NEU!
  end;


  // ===== Bei jedem Aufruf (auch 2., 3., ...) =====
  // Updatefunktion (async, läuft im Hintergrund)
  CheckInternetAsync(WebsiteURL, procedure(IsOnline: Boolean)
  begin
    if IsOnline then
    begin
...
procedure TfMain.AktualisiereAndShowStatistik;
begin
  //Gesamtsttistik EINKÄUFE HEUTE
  //lbEinkaeufeHeuteCount.Caption := IntToStr(GetCountStatistik(FDConnection1, 'inventar', 'Einkaufsdatum', true));

  //Gesamtstatistik EINKÄUFE Edelmetall HEUTE
  //lbEdelmetallEinkaeufeHeuteCount.Caption := IntToStr(GetCountStatistik(FDConnection1, 'ankaufEdelmetall', 'Ankaufsdatum', true));

  //Gesamtstatistik EINKÄUFE GESAMT
  lbEinkaeufeGesamtCount.Caption := IntToStr(GetCountStatistik(FDConnection1, 'inventar', 'Einkaufsdatum', false));


  //Gesamtstatistik VERKÄUFE HEUTE
  //lbVerkaeufeHeuteCount.Caption := IntToStr(GetCountStatistik(FDConnection1, 'inventar', 'Verkaufsdatum', true));

  //Gesamtstatistik VERKÄUFE Edelmetall HEUTE
  //lbEdelmetallVerkaeufeHeuteCount.Caption := IntToStr(GetCountStatistik(FDConnection1, 'verkaufEdelmetall', 'Verkaufsdatum', true));

  //Gesamtstatistik VERKÄUFE GESAMT
  lbVerkaeufeGesamtCount.Caption := IntToStr(GetCountStatistik(FDConnection1, 'inventar', 'Verkaufsdatum', false));
...
begin
  ...
  try
    ...
    // ===== Event temporär deaktivieren =====
    OldOnChange := cbSuchfeld.OnChange;
    cbSuchfeld.OnChange := nil;
    try
      cbSuchfeld.ItemIndex := ini.ReadInteger('SUCHE', 'SpaltenIndex', 0);
    finally
      cbSuchfeld.OnChange := OldOnChange;
    end;

    // ===== NICHT cbSuchfeldSelect aufrufen während FIsLoading! =====
    if not FIsLoading then
      cbSuchfeldSelect(nil);

    ColumnToSort := ini.ReadInteger('SORTIERUNG', 'Spalte', 1);
    if ini.ReadBool('SORTIERUNG', 'SortDirection', True) then
      SortDir := 1
    else
      SortDir := 0;

    SHOWHINWEISFENSTER := ini.ReadBool('ANSICHT', 'ShowDownHintWindow', True);
    SHOWEDELMETALLANVERKAUFWINDOW := ini.ReadBool('ANSICHT', 'showEdelmetallAnVerkaufWindow', False);
    cbShowEdelmetallAnVerkauf.Checked := SHOWEDELMETALLANVERKAUFWINDOW;
  finally
    ini.Free;
  end;
  ...
  // ===== AlphaSort nur wenn nicht im Ladevorgang =====
  if not FIsLoading then
...
begin
  ...
  try
    ...
    try
      ...
      // 2. Import mit Datumskonvertierung
...
procedure TfMain.lbPreisabfrageClick(Sender: TObject);
begin
  fPreisabfrage.Show;
end;


procedure TfMain.lbPreisabfrageMouseEnter(Sender: TObject);
begin
  pnlPreisabfrage.Left := pnlPreisabfrage.Left + 1;
  pnlPreisabfrage.Top := pnlPreisabfrage.Top + 1;
end;


procedure TfMain.lbPreisabfrageMouseLeave(Sender: TObject);
begin
  pnlPreisabfrage.Left := pnlPreisabfrage.Left - 1;
  pnlPreisabfrage.Top  := pnlPreisabfrage.Top  - 1;
end;
...
var
  ...
  Einkaufswert, Verkaufswert, Steuerbetrag: Int64;
begin
  ...
  try
    ...
    try
      ...
      // Optimierte SQL: keine CASTs nötig
      Q.SQL.Text :=
        'SELECT id, Einkaufsdatum, Verkaufsdatum, SKU, Einkaufswert, EinkaufBemerkung, ' +
        'Verkaufswert, VerkaufBemerkung, Steuerbetrag, RechnungsNr, Einheit, AnkaufformularID ' +
        'FROM inventar ' +
        'ORDER BY id DESC';
      ...
      begin
        ...
        Einkaufsdatum := Q.FieldByName('Einkaufsdatum').AsString;
        Verkaufsdatum := Q.FieldByName('Verkaufsdatum').AsString;

        Einkaufswert := Q.FieldByName('Einkaufswert').AsLargeInt;
        Verkaufswert := Q.FieldByName('Verkaufswert').AsLargeInt;
        Steuerbetrag := Q.FieldByName('Steuerbetrag').AsLargeInt;

        SubItems.Add(SQLiteDateToDisplay(Einkaufsdatum));
        SubItems.Add(Q.FieldByName('SKU').AsString);
        SubItems.Add(Q.FieldByName('Einheit').AsString);
        SubItems.Add(Int100ToDecimalString(Einkaufswert));
        SubItems.Add(Q.FieldByName('EinkaufBemerkung').AsString);
        SubItems.Add(SQLiteDateToDisplay(Verkaufsdatum));

        // Verkaufswert (leeren String bei NULL)
        if (Q.FieldByName('Verkaufswert').IsNull) OR (Q.FieldByName('Verkaufswert').Text = '0') then
          SubItems.Add('')
        else
          SubItems.Add(Int100ToDecimalString(Verkaufswert));

        SubItems.Add(Q.FieldByName('VerkaufBemerkung').AsString);

        // Steuerbetrag (leeren String bei NULL)
        if (Q.FieldByName('Steuerbetrag').IsNull) OR (Q.FieldByName('Steuerbetrag').Text = '0') then
          SubItems.Add('')
        else
          SubItems.Add(Int100ToDecimalString(Steuerbetrag));

        SubItems.Add(Q.FieldByName('RechnungsNr').AsString);
        SubItems.Add(Q.FieldByName('AnkaufformularID').AsString);

        // Alle SubItems auf einmal hinzufügen
  ...
  // Letzte Sortierung entfällt, SQL liefert bereits sortiert
  LastSorted := 1;
end;
...
procedure TfMain.LoadEdelmetallAnkaufToListView;
var
  Q: TFDQuery;
  Item: TListItem;
  SubItems: TStringList;
  Ankaufsdatum: string;
  Ankaufswert, Gewicht: Int64;
begin
  lvEdelmetall.Items.BeginUpdate;
  try
    lvEdelmetall.Items.Clear;
    ...
    try
      ...
      while not Q.Eof do
      begin
        Item := lvEdelmetall.Items.Add;
        Item.Caption := '';

        SubItems.Clear;

        Ankaufsdatum := Q.FieldByName('Ankaufsdatum').AsString;
        Ankaufswert := Q.FieldByName('Ankaufswert').AsLargeInt;
        Gewicht := Q.FieldByName('Gewicht').AsLargeInt;

        SubItems.Add(Q.FieldByName('id').AsString);
        SubItems.Add(SQLiteDateToDisplay(Ankaufsdatum));
        SubItems.Add(Int100ToDecimalString(Ankaufswert));
        SubItems.Add(Q.FieldByName('Karat').AsString);
...
procedure TfMain.LoadEdelmetallVerkaufToListView;
var
  Q: TFDQuery;
  Item: TListItem;
  SubItems: TStringList;
...
begin
  lvEdelmetallVerkauf.Items.BeginUpdate;
  try
    lvEdelmetallVerkauf.Items.Clear;
    ...
    try
      ...
      // Alle Sammelverkäufe, bereits sortiert
      Q.SQL.Text :=
        'SELECT id, RechnungsNr, Verkaufsdatum, Verkaufswert, Steuerbetrag ' +
        'FROM verkaufEdelmetall ' +
      ...
      while not Q.Eof do
      begin
        Item := lvEdelmetallVerkauf.Items.Add;
    ...
    lvEdelmetallVerkauf.Items.EndUpdate;
  end;
  ...
  try
    ...
    try
      ...
      Q.SQL.Text :=
        ...
        'Verkaufswert, VerkaufBemerkung, Steuerbetrag, RechnungsNr, Einheit, AnkaufformularID ' +
        'FROM inventar ' +
        'WHERE ' + Spalte + ' LIKE :SUCHBEGRIFF || ''%'' ' +
        'ORDER BY Einkaufsdatum ASC';

      // ===== Parameter setzen =====
      if SameText(Spalte, 'Einkaufsdatum') or SameText(Spalte, 'Verkaufsdatum') then
      begin
        if IsValidDisplayDate(Suchbegriff) then
          Q.ParamByName('SUCHBEGRIFF').AsString := DisplayDateToSQLiteDate(Suchbegriff)
        else
        begin
          ShowMessage('Datum muss im Format TT.MM.JJJJ eingegeben werden');
          Exit;
        end;
      end
      else
      ...
      begin
        ...
        Item.SubItems.Add(SQLiteDateToDisplay(Einkaufsdatum));
        Item.SubItems.Add(Q.FieldByName('SKU').AsString);
        Item.SubItems.Add(Q.FieldByName('Einheit').AsString);

        Item.SubItems.Add(Int100ToDecimalString(Einkaufswert));
        Item.SubItems.Add(Q.FieldByName('EinkaufBemerkung').AsString);

        Item.SubItems.Add(SQLiteDateToDisplay(Verkaufsdatum));

        if (Q.FieldByName('Verkaufswert').IsNull) OR (Verkaufswert = 0) then
          Item.SubItems.Add('')
        else
          Item.SubItems.Add(Int100ToDecimalString(Verkaufswert));


        Item.SubItems.Add(Q.FieldByName('VerkaufBemerkung').AsString);

        if (Q.FieldByName('Steuerbetrag').IsNull) OR (Steuerbetrag = 0) then
          Item.SubItems.Add('')
        else
          Item.SubItems.Add(Int100ToDecimalString(Steuerbetrag));
...
procedure TfMain.lvEdelmetallCheckboxClick(Sender: TObject; iItem: Integer; State: Boolean);
var
  i: Integer;
  AllChecked: Boolean;
begin
  ...
  try
    ...
    for i := 0 to lvEdelmetall.Items.Count - 1 do
      if not lvEdelmetall.Items[i].Checked then
      begin
        AllChecked := False;
        Break;
      end;

    cbCheckUncheckAll.Checked := AllChecked;
...
var
  i: integer;
begin
  i := lvEdelmetallVerkauf.ItemIndex;
  if i <> -1 then
  begin
    fSammelverkaeufe.VERKAUFID := StrToInt(lvEdelmetallVerkauf.Items[i].Caption);
    fSammelverkaeufe.Show;
  end;
end;
...
procedure TfMain.lvInventarCompare(Sender: TObject; Item1, Item2: TListItem; Data: Integer; var Compare: Integer);
begin
    ...
    begin
      ...
      begin
       ...
       //Wenn AnkaufformularID = 0 dann
        ...
        begin
          //Kundenankauf
          fEditKundenankauf.ENTRYID := StrToInt(lvInventar.Items[i].Caption);
          fEditKundenankauf.ShowModal;
        end;
      end
      else
...
initialization
  WebsiteURL  := 'https://programme.ensacom.de/AlexanderKeller/Inventarliste/Version2/';
