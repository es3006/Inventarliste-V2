...
type
  ...
  private
    procedure LoadData;
    procedure RefreshSteuerbetrag;
    procedure UpdateVerkaufInDB(const AID: Integer; const ADelete: Boolean);
...
procedure TfVerkauf.UpdateVerkaufInDB(const AID: Integer; const ADelete: Boolean);
var
  i: Integer;
  FDQuery: TFDQuery;
  verkaufswertCent: Int64;
  steuerbetragCent: Int64;
  rechnungsNr, verkaufBemerkung: string;
begin
  i := fMain.lvInventar.ItemIndex;
  if (AID <= 0) or (i = -1) then
  begin
  ...
  try
    ...
    try
      ...
      begin
        // Alles zurücksetzen
        FDQuery.ParamByName('VERKAUFSDATUM').Clear;
        FDQuery.ParamByName('VERKAUFSWERT').AsLargeInt   := 0;
        FDQuery.ParamByName('VERKAUFBEMERKUNG').AsString := '';
        FDQuery.ParamByName('STEUERBETRAG').AsLargeInt   := 0;
        FDQuery.ParamByName('RECHNUNGSNR').AsString      := '';
      end
      else
      begin
        // Werte aus UI holen
        ...
        SetSQLiteDateParam(
          FDQuery.ParamByName('VERKAUFSDATUM'),
          dtpVerkaufsdatum
        );
  ...
  with fMain.lvInventar.Items[i] do
  begin
    if ADelete then
    begin
      SubItems[5] := '';
    ...
    begin
      SubItems[5] := SQLiteDateToDisplay(DateTimeToSQLiteDate(dtpVerkaufsdatum.Date));
      SubItems[6] := Int100ToDecimalString(DecimalStringToInt100(edVerkaufswert.Text));
      SubItems[7] := Trim(edVerkaufBemerkung.Text);
      SubItems[8] := Int100ToDecimalString(DecimalStringToInt100(edBesteuernderBetrag.Text));
      SubItems[9] := Trim(edRechnungsNr.Text);
    end;
  end;
...
begin
  ...
  UpdateVerkaufInDB(ENTRYID, False);
...
procedure TfVerkauf.RefreshSteuerbetrag;
var
  VerkaufsCent, EinkaufsCent, DiffCent: Int64;
begin
  // ===== Formatierung mit Helper-Funktion =====
  // Konvertiere zu Int64 und zurück zu String für einheitliche Formatierung
  edBesteuernderBetrag.Text := Int100ToDecimalString(DecimalStringToInt100(edBesteuernderBetrag.Text));
...
begin
  ...
  begin
    Caption := 'Warenverkauf';
    Label5.Caption := 'Warenverkauf';
    btnDeleteverkauf.Visible := false
  end
  else
  begin
    Caption := 'Warenverkauf bearbeiten';
    Label5.Caption := 'Warenverkauf bearbeiten';
...
begin
  ...
  try
    ...
    Q.SQL.Text :=
      'SELECT id, Einkaufsdatum, SKU, Einkaufswert, Artikelname, EinkaufBemerkung, ' +
    ...
    begin
      ...
      // Datum
      LoadSQLiteDateToDTP(Q.FieldByName('Verkaufsdatum').AsString, dtpVerkaufsdatum);

      // Verkaufswert und Steuerbetrag
      verkaufswert := Q.FieldByName('Verkaufswert').AsLargeInt;
      steuerbetrag := Q.FieldByName('Steuerbetrag').AsLargeInt;

      if (verkaufswert = 0) and (steuerbetrag <> 0) then
        edVerkaufswert.Text := '0,00'
      else
        edVerkaufswert.Text := Int100ToDecimalString(verkaufswert);

      edBesteuernderBetrag.Text := Int100ToDecimalString(steuerbetrag);
    end;
  finally
    Q.Free;
  end;
